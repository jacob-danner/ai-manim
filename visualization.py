...
class InitialResidualStreams(Scene):
    def construct(self):
        ...
        # 1. Title
        title = Text(
            "Initial Residual Streams", font_size=36, font="sans-serif", color=BLACK
        ).to_corner(UL)
        self.add(title)
        ...
        def create_segmented_block(
            color_segments, outline_color=BLACK, fill_opacity=1.0
        ):
            segments = VGroup()
            for i in range(8):
        ...
        white_block.move_to(ORIGIN).shift(
            RIGHT * 2
        ...
        purple_block_colors = [PURPLE_B] * 4 + [
            None
        ] * 4  # Top 4 purple, bottom 4 white
        ...
        top_row_labels = VGroup(batch_label, tokens_label, dimension_label).arrange(
            RIGHT, buff=1.0
        )

        # Arrange the values horizontally
        bottom_row_values = VGroup(batch_value, tokens_value, dimension_value).arrange(
            RIGHT, buff=1.0
        )
        ...
        dimension_group = VGroup(top_row_labels, bottom_row_values).arrange(
            DOWN, buff=0.5
        )
